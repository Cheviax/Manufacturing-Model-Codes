            ...
            for y in y_:
                  ...
                  for x in x_:
                        ...
                        for i in np.arange(npar):
                              ...
                              if I and (int(particle[4]) == 1):
                                    M[xi,yi,zi] += 1
                              elif I and (int(particle[4]) > 1):
                                    M[xi,yi,zi] += 100
...
#Calculate highest z value  --> Questo da usare per not periodic conditions
#zmax = np.amax(part[:,2])
#npar_ghost = np.shape(atoms)[0]

C = get_image(M,npar_ghost,voxel,Lxm,Lxp,Lym,Lyp,Lzm,Lzp)
