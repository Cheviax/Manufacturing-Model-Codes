...
//#define ETA_EXP2 293
#define DENSITY_EXP 2.30
#define VISCOSITY1_EXP 7480
#define VISCOSITY2_EXP 2860

#define LEARNING_PSO
#define PSO_WITH_HUMAN_INITIALIZATION
...
bool fill_from_learning(const string& filename, vector<vector<double> >& v_res)
{
  ifstream flux(filename);
  ...
  while(not flux.eof())
    {
      ...
      while(true)
	{
	  size_t i_new = line.find(',', i);
	  string s_val = line.substr(i, i_new-i);
	  //cerr<<s_val<<endl;  //sleep(1);
	  

	  double v = -1;
	  if(stringtools_toDouble(s_val, v))
	    {
	      v_val.push_back(v);
	    }
	  else
...
double reevaluate(const vector<double>& v_data)
{
  assert(v_data.size()==18);
  
  double density = v_data[11];
  double viscosity1 = v_data[12];
  double viscosity2 = v_data[13];
  
  double density_norm = density/DENSITY_EXP;
  double viscosity1_norm = viscosity1/VISCOSITY1_EXP;
  double viscosity2_norm = viscosity2/VISCOSITY2_EXP;
  
  return 0.3333*pow(1-density_norm, 2)+0.3333*pow(1-viscosity1_norm, 2)
    +0.3333*pow(1-viscosity2_norm, 2);
}
	    
void get_nearest_neighbours_for_pso(const vector<vector<double> >& m_data, vector<vector<double> >& v_best)
{
  vector<double> v_eval;

  double val_max = 0;
  for(size_t i = 0; i<m_data.size(); i++)
    {
      v_eval.push_back(reevaluate(m_data[i]));

      assert(i<v_eval.size());
      if((i==0)||(val_max<v_eval[i]))
	val_max = v_eval[i];
    }
  
  val_max = val_max+1;

  size_t nb_points_for_pso = 8;
  
  for(size_t j = 1; j<=nb_points_for_pso; j++)
    {
      size_t best_index = 0;
      double best_eval = 0;
      for(size_t i = 0; i<v_eval.size(); i++)
	{
	  if((i==0)||(best_eval>v_eval[i]))
	    {
	      best_index = i;
	      best_eval = v_eval[i];
	    }
      ...
      assert(best_index<m_data.size());
      v_best.push_back(m_data[best_index]);
      assert((v_best.back()).size()>2);
      (v_best.back()).resize(v_best.back().size()-1);
      (v_best.back())[v_best.back().size()-1] = v_eval[best_index];
      v_eval[best_index] = val_max; 
    }
}
...
public:
  ...
  void init_random_position(unsigned int dim, const vector<double>& v_lb, const vector<double>& v_ub)
  {
	...
	cerr<<"random - "<<lb<<" "<<ub<<" "<<position[i]<<endl; //sleep(1);
      }
  }
  ...
  void init_position(const vector<double>& pos)
  {
    position = pos;
  }
  ...
  void init_random_velocity(unsigned int dim, const vector<double>& v_lb, const vector<double>& v_ub)
  {
	...
	cerr<<"random - "<<lb<<" "<<ub<<" "<<velocity[i]<<endl; //sleep(1);
      }
  }
...
void multithread_pso(const PSO_Problem& problem, const PSO_Parameters& params, PSO_Out& out)
{
...

  {
    ...
    string filename = "results_0.csv";
    vector<vector<double> > m_data;
    if(fill_from_learning(filename, m_data))
      {
	
#ifdef LEARNING_PSO 
	
	cerr<<"[ LEARNING PSO ]"<<endl;
	
	if(true)
	  {
	    ofstream flux_oldpos("particules_before.csv");
	    
	    for(size_t i = 0; i<m_data.size(); i++)
	      {
		for(size_t j = 0; j<m_data[i].size(); j++)
		  {
		    flux_oldpos<<m_data[i][j];
	...
	//cerr<<m_data.size()<<endl;
	//for(size_t i = 0; i<m_data.size(); i++)
	//{
	//cerr<<m_data[i].size()<<endl;
	//}
	
	vector<vector<double> > v_points0;
	get_nearest_neighbours_for_pso(m_data, v_points0);
	
	assert(v_particles.size()==v_points0.size());
	for(size_t j = 0; j<v_points0.size(); j++)
	  {
	    //position 2 -> 9
	    //cost 16
	    
	    vector<double> my_point;
	    for(size_t k = 2; k<=9; k++)
	      {
		assert(k<v_points0[j].size());
		my_point.push_back(v_points0[j][k]);
	      }
	    
	    v_particles[j].init_position(my_point);
	    v_particles[j].init_random_velocity(nbVar, minVelocity, maxVelocity);
	    
	    //Simulate evaluation
	    assert(16<v_points0[j].size());
	    v_particles[j].cost = v_points0[j][16];
...
#ifdef PSO_WITH_HUMAN_INITIALIZATION
	...
	for(size_t j = 0; j<v_particles.size(); j++)
	  {
	    ...
	    v_threads0.push_back(std::thread(compute_one_initialization_of_pso, j, nbVar, std::ref(v_particles),
					     varMin, varMax, minVelocity, maxVelocity, filenamesList,
...
#endif //PSO_WITH_HUMAN_INITIALIZATION
      }
...
/*
//PSO Buggue (parentheses+r1+ init random velocity)
  ...
  for(size_t i = 0; i<maxIterations; i++)
      ...
      for(size_t j = 0; j<nbPop; j++) 
	{
	  ...
	  for(size_t h = 0; h<v_r1.size(); h++)
	    {
	      v_social[h] =
		c2*v_r1[h]*global_best_position[h]-v_particles[j].position[h];
	    }
  ...
  if(false)
    {
      ...
      fill_from_learning(filename, m_data);
      ...
      vector<vector<double> > v_best;
      
      get_nearest_neighbours_for_pso(m_data, v_best);

      for(size_t i = 0; i<v_best.size(); i++)
	{
	  for(size_t j = 0; j<v_best[i].size(); j++)
	    {
	      cerr<<v_best[i][j]<<" ";
	    }
